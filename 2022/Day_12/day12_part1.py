...
def explore(map:list, pos: tuple, history: list):
  ...
  if shortest_len and len(new_history) > shortest_len:
    return history_list

  if height == "E":
    print(f"FOUND E AT {new_history}")
    if shortest_len is None or len(new_history) < shortest_len:
      shortest_len = len(new_history)
    history_list.append(new_history)
    return history_list

  north_pos = (pos[0], pos[1] - 1) if pos[1] - 1 >= 0 and (pos[0], pos[1] - 1) not in new_history else None
  east_pos = (pos[0] + 1, pos[1]) if pos[0] + 1 < map_size[0] and (pos[0] + 1, pos[1]) not in new_history else None
  south_pos = (pos[0], pos[1] + 1) if pos[1] + 1 < map_size[1] and (pos[0], pos[1] + 1) not in new_history else None
  west_pos = (pos[0] - 1, pos[1]) if pos[0] - 1 >= 0 and (pos[0] - 1, pos[1]) not in new_history else None
  ...
  if north_pos and abs(ord(height) - ord(north_height)) <= 1:
    history_list += explore(map, north_pos, new_history)
  if east_pos and abs(ord(height) - ord(east_height)) <= 1:
    history_list += explore(map, east_pos, new_history)
  if south_pos and abs(ord(height) - ord(south_height)) <= 1:
    history_list += explore(map, south_pos, new_history)
  if west_pos and abs(ord(height) - ord(west_height)) <= 1:
